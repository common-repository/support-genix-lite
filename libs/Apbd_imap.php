...
class Apbd_imap {
	...
	{
		$enc = '';
		if ($config['encrypto'] != NULL && isset($config['encrypto']) && $config['encrypto'] == 'S')
		{
			$enc = '/imap/ssl/novalidate-cert';
		}
		elseif ($config['encrypto'] != NULL && isset($config['encrypto']) && $config['encrypto'] == 'T')
		{
			$enc = '/imap/tls/novalidate-cert';
		}elseif ($config['encrypto'] != NULL && isset($config['encrypto']) && $config['encrypto'] == 'A')
        {
            $enc = '/imap/tls';
        }
		...
		if (false === $this->stream)
		{
            ...
            $configbk=$config;
            $configbk['pass']='hidden';
			$msg=json_encode($config)."<br/>".$obj->_e("Error Message:")."<br/>".$this->last_error();
            Mapbd_wps_debug_log::AddGeneralLog("Email to ticket imap connection config error", $msg);
			return $msg;
		}
    ...
    /**
     * @param Mapbd_wps_imap_settings $db_imap
     * @return bool|string
     */
    public function imap_connect_by_db($db_imap)
    {
        $config = array(
            'host' => "{$db_imap->wps_host}:{$db_imap->wps_port}",
            'encrypto' => $db_imap->secure_protocol == "A" ? $db_imap->secure_protocol_type : null,
            'user' => trim($db_imap->user_email),
            'pass' => $db_imap->user_password
        );
        return @$this->imap_connect($config);
    }
	/**
	...
	public function get_attachment($id = 0, $index = 0, $tmp_path = '')
	{
		...
		$message = imap_fetchbody($this->stream, $id, $attachment['partNum']);
	...
	protected function format_message($id = 0, $with_body = TRUE, $embed_images = TRUE)
	{
		...
		$email = [
			...
			'uid'      => $uid,
			'id'       => $id,
			'reply_to' => !empty($header->reply_to[0]) ? $this->to_address($header->reply_to[0]) : '',
		...
		if (count($attachments) > 0)
		{
			foreach ($attachments as $val)
			{
				...
				if (isset($email['attachments']) && is_array($email['attachments']) && (count(@$email['attachments']) > 0) && (@$email['html'] == TRUE) && ($embed_images == TRUE))
		{
	...
	{
		return false !== $this->stream ? imap_mailboxmsginfo($this->stream) : FALSE;
	}
