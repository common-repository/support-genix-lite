	...
	abstract class AppsBDKarnelLite {
		...
		final function CheckPluginVersionUpdate() {
            ...
            if ( empty( $db_version ) || $db_version != $this->pluginVersion ) {
                update_option( "APBD_pv_" . $this->pluginSlugName, $this->pluginVersion );
		...
		public static function GetError( $prefix = '', $postfix = '' ) {
			if ( !empty(self::$errorMessage) && count( self::$errorMessage ) > 0 ) {
		...
		public static function GetInfo( $prefix = '', $postfix = '' ) {
			if ( !empty(self::$infoMessage) && count( self::$infoMessage ) > 0 ) {
		...
		public static function GetWarning( $prefix = '', $postfix = '' ) {
			if ( !empty(self::$warningMessage) && count( self::$warningMessage ) > 0 ) {
		...
		static function IsMainOptionPage() {
			$file = basename(sanitize_text_field($_SERVER['SCRIPT_FILENAME']));
			if ( $file == "plugins.php" ) {
		...
		final function LinksActions( $links ) {
			$links[] = "<a class='edit coption' href='admin.php?page=" . $this->pluginSlugName . "'>" . $this->__( "Settings" ) . "</a><br/><br/>";
			foreach ( $this->moduleList as $moduleObject ) {
		...
		final function OnActive() {
			...
			update_option( 'apbd_support_genix_lite_redirect_flag', true );
		}
                                ...
                                <div id="content" class="pos-relative pt-0">
                                    ...
                                    <div id="apbd-app-loader" class="apbd-app-loader ">
                                        <div class="text-center" id="apbd-app-waiting">
                                            <img class="mh-50" src="<?php echo plugins_url( "images/lighboxloader.svg", $this->pluginFile ); ?>"
                                               alt="<?php $this->_e( "Loading.." ); ?>">
                                            <br>
		...
		final function RedirectToSettings() {
			$redirect_flag = get_option( 'apbd_support_genix_lite_redirect_flag' );

			if ( true === rest_sanitize_boolean( $redirect_flag ) ) {
				update_option( 'apbd_support_genix_lite_redirect_flag', false );
				wp_safe_redirect( admin_url( 'admin.php?page=support-genix-lite' ) );
				exit();
			}
		}
