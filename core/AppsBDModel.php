...
if (!class_exists("AppsBDModel")) {
    class AppsBDModel
    {
        ...
        protected $cacheTime = 300;         protected $kernelObject = NULL;
        ...
        function getTextByKey($property, $isTag = true, $key = NULL)
        {
            ...
            if (!empty($key) || property_exists($this, $property)) {
	            ...
	            return esc_html($this->__("Undefined Property"));
            }
        }
        ...
        public function IsValidForm($isNew = true, $addError = true)
        {
            ...
            foreach ( $this->validations as $key => $value ) {
                if ( $isNew || ( isset( $this->setProperties [ $key ] ) && empty( $this->setOption [ $key ] ) ) ) {
                    ...
                    if ( ( 'meta_value' === $key ) && is_serialized( $this->$key ) ) {
                        continue;
                    }
                    ...
                    if ( is_array( $rules ) ) {
                        ...
                        if($this->IsHTMLProperty($key)){
                            $this->{$key}(wp_kses_post($this->$key));
                        }else{
                            $this->input_clean($this->$key);
                            if(!empty($this->$key)) {
                                $textData = wp_strip_all_tags($this->$key);
                            }else{
                                $textData = $this->$key;
                            }
                            $this->{$key}(sanitize_text_field($textData));
                        }
                        ...
                        if ( in_array( "lowercase", $rules ) ) {
                            $this->$key(strip_tags( strtolower( $this->$key ))) ;
                            ;
                        }
                        ...
                        if ( ! empty( $this->$key ) && in_array( "resize_length", $rules ) && isset( $bracketValue['resize_length'] ) ) {
                            $resizeLength=(int) $bracketValue['resize_length'];
                            if ( strlen( $this->$key ) < $resizeLength ) {
                                $this->$key(substr($this->$key,$resizeLength));
                            }
                        }
            ...
            foreach ($dataarray as $key => $value) {
                if (property_exists($this, $key)) {
                    ...
                    if($isHtml){
                        ...
                        if(!empty($oldValue)) {
                            $oldValue = wp_kses_post($oldValue);
                        }
                    }else{
                        $NewValue = $value;
                        $oldValue = $this->PostValue( "old_" . $key );
        ...
        function SetFromPostData($isNew = false)
        {
            ...
            foreach ($postData as $key => &$value) {
                $value = in_array($key, $this->htmlInputField) ? wp_kses_html($value) : sanitize_text_field($value);
            }
            return $this->SetFromArray($postData, $isNew);
        }
        ...
        static function GetTotalQueries()
        {
            ...
            <div class="row">
                <div class="panel panel-info">
                    <div class="panel-heading"><?php $obj->_e("Queries") ; ?></div>
